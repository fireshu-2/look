...
// 🌟 核心修复：将 LUT 文件读取到内存中
	if (lut_path != nullptr && strlen(lut_path) > 0) {
		...
		if (fp != nullptr) {
			...
			size_t read_bytes = fread(m_pcolor_lut, 1, std::min<size_t>(m_file_size, 1024), fp);
			if (read_bytes != std::min<size_t>(m_file_size, 1024)) {
	...
	for (int i = 0; i < OSD_LAYER_COUNT; i++) {
		...
		if (osd_create_layer(m_osd_handle, (ssLAYER_HANDLE)i, &layer) == 0) {
			...
			osd_enable_layer(m_osd_handle, (ssLAYER_HANDLE)i, i < 2 ? true : false); // 默认开启显示
		} else {
